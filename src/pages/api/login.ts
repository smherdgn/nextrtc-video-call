...
import { signAccessToken, signRefreshToken, ACCESS_TOKEN_NAME, REFRESH_TOKEN_NAME, ACCESS_TOKEN_EXPIRY, REFRESH_TOKEN_EXPIRY } from '@/lib/authUtils';
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (loginSuccess && userPayload) {
    ...
    const refreshTokenMaxAge = parseInt(REFRESH_TOKEN_EXPIRY.slice(0, -1)) * (REFRESH_TOKEN_EXPIRY.endsWith('d') ? 86400 : REFRESH_TOKEN_EXPIRY.endsWith('h') ? 3600 : 1);
